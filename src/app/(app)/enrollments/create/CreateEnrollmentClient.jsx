...
export default function CreateEnrollmentClient({ activeYear, allYears, levels }) {
  ...
  return (
    <div className="card" style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div className="card-header d-flex justify-between align-center" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>Nueva Matrícula</h2>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="form-group">
            <label className="form-label" htmlFor="anio_id">Año Escolar</label>
            <select className="form-control" name="anio_id" required value={formData.anio_id} onChange={(e) => setFormData({...formData, anio_id: e.target.value})}>
              {allYears?.map(y => (
                <option key={y.id} value={y.id}>{y.anio}</option>
              ))}
            </select>
          </div>
        ...
        <div className="text-right mt-4 pt-4" style={{ borderTop: '1px solid var(--border-color)', textAlign: 'center' }}>
          <button type="submit" className="btn btn-success btn-block py-3" style={{ fontSize: '1.1rem', background: '#10b981', color: 'white', width: '100%', padding: '12px' }}>
            <i className='bx bx-check-circle'></i> Procesar Matrícula
          </button>
        </div>
      </form>
